...
- (instancetype)initWithSize:(CGSize)size{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(stopAction) name:@"stopNotification" object:nil];
    }
    return self;
}
...
//    [self runAction:[SKAction repeatActionForever :[SKAction playSoundFileNamed:@"game_music.mp3" waitForCompletion:YES]] withKey:@"repeatActionForever"];
    [self prepAudio];
}
- (void) prepAudio{
//    NSError *error;
...
- (void)initScroe{
    ...
    _scoreLabel.position = CGPointMake(70 , self.size.height - 52);
